...
int socket_handler(struct __sk_buff *skb)
{
	...
	// reserve space for the captured packet from the BPF ringbuf.
	...
	__u32 data_len = 24; // TODO: Remove this magic number.
	if (data_len <= skb->len) {
		bpf_skb_load_bytes(skb, nhoff + ((verlen &0xF) << 2), e->data, data_len);
	}
