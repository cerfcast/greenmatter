...
public:
  ReadPower(chip::ClusterId clusterId, chip::AttributeId attributeId,
            chip::NodeId destinationId, chip::EndpointId endpointId,
            CredentialIssuerCommands *credsIssuerConfig)
      : ReportCommand("NA", credsIssuerConfig), mClusterIds(1, clusterId),
  ...
  CHIP_ERROR SendCommand(chip::DeviceProxy *device,
                         std::vector<chip::EndpointId> endpointIds) override {
    ChipLogProgress(chipTool, "About to send a read power command");
    return ReportCommand::ReadAttribute(device, endpointIds, mClusterIds,
                                        mAttributeIds);
  }

  bool Success() const { return mError == CHIP_NO_ERROR; }

  int32_t Get() const { return mValue; }
...
int currentCurrent(chip::NodeId destinationId, chip::EndpointId endpointId) {
  ...
  auto readVoltageAttributeCommand = ReadPower(
      Id, Attributes::ActiveCurrent::Id, destinationId, endpointId, &credIssuerCommands);
